...
class IndexController extends AbstractController
{
    ...
    public function user($id)
    {
         ...
         */
         $userModel   = new Model\User(); // Instantiate new User model object
         $user  = $userModel->getById($id); // Get a user by ID
         if(count($user) == 0){
             // Message if the ID passed does not return a user.
             $view->title = 'User Does NOT Exist';
         } else {
